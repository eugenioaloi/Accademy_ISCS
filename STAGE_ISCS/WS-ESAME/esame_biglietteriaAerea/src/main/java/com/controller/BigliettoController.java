...
public class BigliettoController {
	...
	public ResponseEntity<Void> addBiglietto(@RequestBody BigliettoInfo info,UriComponentsBuilder builder){
		...
		if(!flagQt) {
			return new ResponseEntity<Void>(HttpStatus.CONFLICT);//409 non è permesso procedere
		}
	...
	public ResponseEntity<Void> updateBiglietto(@RequestBody BigliettoInfo info,UriComponentsBuilder builder){
		...
		// 1) setto i parametri passati dall'info, ovvero il json
		bl.setQtrichiesta(info.getQtrichiesta());
		
		// 2) aggiorno il db per il volo corrispondente e il biglietto corrispondente
		service.updateDisponibilita(bl);
