...
//		Article obj = articleRepository.findById(articleId).get();
		Article obj = articleRepository.findById(articleId).orElse(new Article());
		return obj;
	}
	...
	public synchronized boolean addArticle(Article article) {
		List<Article> list = articleRepository
				.findByTitleAndCategory(article.getTitle(), article.getCategory());
		if (list.size() > 0) {
			return false;
		} else {
